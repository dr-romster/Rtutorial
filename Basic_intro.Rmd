...
Not all text is of equal standing. R does not know how to differentiate text that represents categories from just standard text terms. 

We can covert data (text or numerical) to categeroies using the `factor()` function. This function can also take the argument `levels =` where you provide a list of categories that you wish to assign, in the the order that you want. 

If you stipulate a level that a term that the text or numerical data does not correspond to then that data point will be attributed a missing value `NA`. 

Changing the sex column to a factor (categorical data type):
