...
institute: "NIHR DSE Fellow <br/> University of Cambridge"
date: "`r Sys.Date()`"
output:
...
.title3[NIHR DSE Fellow, Consultant NCCU & Anaesthetics  <br/>
MRC Biostatistics Unit & Addenbrooke's Hospital <br>
<br>
...
No generative AI was used for this workshop.

---

class: highlight-last-item

# In 2 hours you will be able to 

- Understand some of the basics of `R`

--

- Create variables, Data frames

--

- Use functions, do some plotting
...
- Understand broader data science principles
...
Cambridge & Imperial
<br/><br/>
 FY, Core medicine, Sydney, Core anaesthetics
<br/><br/>
 St3 Anaesthetics East of England
<br/><br/>
 *SMART course*
<br/><br/>
...
<br/><br/>
  Dual CCT
<br/><br/>
NIHR DSE Fellowship, NCCU Consultant post
...
.center[ <img src="img/ANRO.png" alt="ANRO" width="610"/>]



.right[**The Register** 12th October 2023] 
<br/>
<br/>
<br/>
...
# Why can't I just use <s>Excel</s> ChatGPT?

Large Language Models (LLMs) are **very good** at entry and intermediate level coding in many programming languages.

They are excellent at translating between different languages.

They really help with error messages

They really help you to understand code

RTutor.AI (https://rtutor.ai/)

Domain knowledge is still **essential** when using AI tools.
...
## https://posit.cloud/content/4956373

.center[<img src="img/SmartR_QR.png" width = 400/>]
...
# Getting Started
.footnote[https://posit.cloud/content/4956373]
...
## In Posit (R-Studio) cloud &nbsp; &nbsp; &nbsp; .tiny[       https://posit.cloud/content/4956373 ] 
...
<br/>
<br/>
Multiply your numerical variable by 2
<br/>
<br/>
Add your name to your string variable
<br/>
<br/>
Add a 4th number to your numerical variable
]
--
...
<br/>
containing the strings: Harry, Hermione, Ron

{{content}}

--
<br/>
consisting of the numbers: 10, 50, 5
...
```{r echo = FALSE}
set.seed(42)
df_ex <- knitr::kable(head(data.frame("Patient ID"= c(1, 1, seq(1:8)),
                                    "Day" = c(1, 2, 5, rep(1, 7)),  
                        "Hb" = round(rnorm(10, 10, 2),1),
                        "SBP" = round(rnorm(10, 110, 30),0),
                        "WCC" = round(rnorm(10, 8, 3),1),
...
.red[What's going on with Patient 1's Hypertension and Diabetes diagnosis?]
...
# Data frames  &nbsp; &nbsp; &nbsp; .tiny[       https://posit.cloud/content/4956373 ] 

CSV files can be imported using the `read.csv()` function. 

```{r echo=T, eval=F}
imported_df <- read.csv(file = "external_data.csv", header = TRUE)
imported_df
```


Spreadsheet files can be imported but require third-party packages like `readxl` and `googlesheets4`. 

```{r echo=T, eval=F}
library(readxl)
sample_rota <- read_xlsx(path = "sample_rota.xlsx")

library(googlesheets4)
path_to_google_sheet <- "https://docs.google.com/spreadsheets/d/11MIsxVLAv_Nw8lVJbEUSYJ7X1bUTiGyQLA6GXzKB7v4/edit?usp=sharing"

sample_nccu_rota <- googlesheets4::read_sheet(path_to_google_sheet, 
                                              na = "", 
                                              range = "Generic pattern!A3:X93")
...
If you are still stuck then Google / StackOverflow / ChatGPT are your friends
{{content}}
]
---
...
.right-column[
.footnote[https://posit.cloud/content/4956373]
Create a data frame using the two variables (one with character strings, the other containing some numbers) into a **data frame**
{{content}}
]
...
<br/><br/>
Change the value at this location to 50
...
.center[<img src="img/ferritin_log_odds.png" width="600"/>]
<br/>
Mehta P, Samanta R et al *Thorax* 2024
---

class: center, middle, inverse

.center[<img src="img/tbi3.png" width="800"/>]
<br/>
Samanta R, Chiollaz AC, Needham E et al *eBiomed* 2024

---

class: center, middle, inverse

.center[<img src="img/infection_to_slide.png" width="1000"/>]
...
.center[<img src="img/NCCU_weekly_rota_algorithm.png" width="550"/>]
...
.center[<img src="img/bbc_ggplot.jpg" width="1000"/>]
...
Think of what additional, clinically important, questions you could answer using this data.
...
knitr::include_graphics("img/data_scienctist.jpg")
```
]
...
.right-column[
  ...
  Manipulate the values in a variable using functions and operations<br/><br/>
...
.center[<img src="img/vibe_coder.png" width="1000"/>]
...
Google sheets can be read using the `googlesheets` package. 

--

Data frames can be stored as `.csv` files using `write.csv()`, <br/> or as `.Rdata` objects using the `save()` function.

--

`.Rdata` files can contain multiple objects <br/> e.g. variables, data frames, models, plots.
...
.center[<img src="https://cdn.myportfolio.com/45214904-6a61-4e23-98d6-b140f8654a40/85520b8f-4629-4763-8a2a-9ceff27458bf_rw_1920.jpg?h=21007b20ac00cf37318dca645c215453" width="1000"/>]

Openscapes blog (c) Julia Lowndes & Allison Horst
...
---
## Tidy data exercise  &nbsp; &nbsp; &nbsp; .tiny[       https://posit.cloud/content/4956373 ] 

.left-column[
Create a new <br/> script file 
<br/><br/>
Load the `dplyr` <br/> package
<br/>
<br/>
Load the `readxl` <br/> package
<br/>
<br/>
Import the `sample_rota.xlsx` file
<br/>
<br/>
`View()` the imported <br/> object
...
<br/>
Use the `New File` button above the script pane
{{content}}
]
<br/><br/>
...
<br/><br/>
Use the `readxlsx()` function from the `readxl` package
{{content}}
...
<br/>
<br/>
We want to work out who is on shift next Monday.

`r countdown::countdown(minutes = 3, warn_when=60)`
{{content}}

---
## Tidy data exercise  &nbsp; &nbsp; &nbsp; .tiny[       https://posit.cloud/content/4956373 ] 

We want to work out who is on shift .red[next Monday].

--

We need to create two objects: 

- One with the dates and shifts
- One with the doctor names
- They need to have a common element

--

We are going to `filter` the rota table

--

We are going to `join` the doctor table to it

--

We may need to .red[pivot] our tables to get them into the correct, .red[tidy] shape to do this. 

.center[<img src="img/tidyr_spread_gather_updated.png" alt="gather gif" height="200"/> ]


`r countdown::countdown(minutes = 6, warn_when=60)`
...
Write a report using `R Markdown` 

--

Use AI to help you when stuck

--

A **support community **

- NHS-R, Stack Overflow 

- Blue Sky `#TidyTuesday`
...
<img src="img/hearing.png" alt="tidy Tuesday" width="500"/>

`r fontawesome::fa("twitter")`  `#TidyTuesday` [week of 23 October 2023](https://x.com/samiasab90/status/1720490541442355452?s=20)
...
Data science is an accessible way to do research <br/> compared with experimental work <br/>(which it does not replace).
...
<br/>
<br/>
`r fontawesome::fa("inbox")` rs307@cam.ac.uk
<br/>
`r fontawesome::fa("github")` dr-romster
<br/>
`r fontawesome::fa("linkedin")`
<!-- https://rstudio.github.io/fontawesome/articles/icon-reference.html -->
